...
void MCP4X::output2(unsigned short data_A, unsigned short data_B) {
	...
	//MYSERIAL.println(data_A);

	this->output(MCP4X_CHAN_B, data_B);

	//MYSERIAL.println(data_B);
...
void MCP4X::write(unsigned int data) {
	...
	if (MCP4X_PORT_WRITE)
		PORTB &= 0xef; // 0xef for mega and 0xfb for Uno, Clear PORTB pin 2 = arduino pin 10
	else
	...
	if (MCP4X_PORT_WRITE)
		PORTB |= (1 << 4); // << 4 for mega and << 2 for Uno, set PORTB pin 2 = arduino pin 10
	else
